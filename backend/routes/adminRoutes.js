...
router.post('/colleges', async (req, res) => {
    const { college_name, degree, district, fees, description } = req.body;
    try {
...
router.put('/colleges/:id', async (req, res) => {
    ...
    try {
        await db.execute(
            'UPDATE colleges SET college_name=?, degree=?, district=?, fees=?, description=? WHERE college_id=?',
            [college_name, degree, district, fees, description, req.params.id]
        );
